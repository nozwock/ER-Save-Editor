pub mod events_view_model {
    ...
    impl EventsViewModel {
        pub fn from_save(slot:& SaveSlot) -> Self {
            ...
            let id_to_offset_lookup = EVENT_FLAGS.lock().expect("Lock shouldn't be poisoned");

            // Graces
            for (key, value) in GRACES.lock().expect("Lock shouldn't be poisoned").iter() {
                ...
                // note: not sure panic by expect was delebrate here
                if let Some(it) = events_vm.grace_groups.get_mut(&value.0) {
                    it.push(*key);
                    it.sort();
                }
            }
            ...
            // Whetblades
            for (key, value) in WHETBLADES.lock().expect("Lock shouldn't be poisoned").iter() {
            ...
            // Cookbooks
            for (key, value) in COOKBOKS.lock().expect("Lock shouldn't be poisoned").iter() {
                let event_flag_info = id_to_offset_lookup[&value.0];
                let on = get_bit(slot.event_flags.flags[event_flag_info.0 as usize], event_flag_info.1);
            ...
            // Maps
            for (key, value) in MAPS.lock().expect("Lock shouldn't be poisoned").iter() {
                let event_flag_info = id_to_offset_lookup[&value.0];
                let on = get_bit(slot.event_flags.flags[event_flag_info.0 as usize], event_flag_info.1);
            ...
            // Bosses
            for (key, value) in BOSSES.lock().expect("Lock shouldn't be poisoned").iter() {
                let event_flag_info = id_to_offset_lookup[&value.0];
                let on = get_bit(slot.event_flags.flags[event_flag_info.0 as usize], event_flag_info.1);
            ...
            // Summoning Pools
            for (key, value) in SUMMONING_POOLS.lock().expect("Lock shouldn't be poisoned").iter() {
                let event_flag_info = id_to_offset_lookup[&value.0];
                let on = get_bit(slot.event_flags.flags[event_flag_info.0 as usize], event_flag_info.1);
            ...
            // Colosseums
            for (key, value) in COLOSSEUMS.lock().expect("Lock shouldn't be poisoned").iter() {
                let event_flag_info = id_to_offset_lookup[&value.0];
                let on = get_bit(slot.event_flags.flags[event_flag_info.0 as usize], event_flag_info.1);

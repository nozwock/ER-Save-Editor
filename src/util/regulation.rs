...
impl Regulation {
    pub fn init_params(save: &Save) -> anyhow::Result<()> {
        let res = Regulation::params_from_regulation(save.save_type.get_regulation())?;
        *PARAMS.write().expect("Lock shouldn't be poisoned") = res;
    ...
    where
        T: Default + Clone
    {
        PARAM::<T>::from_bytes(&PARAMS.read().expect("Lock shouldn't be poisoned")[param])
            .expect("Param bytes must be valid")
            .rows
            .into_iter()
            .map(|row| (row.id, row))
        ...
        T: Default + Clone
    {
        rows.iter_mut().for_each(|(_, entry)| {
            entry.name = match map.lock().expect("Lock shouldn't be poisoned").get(&(entry.id)) {
    ...
    pub fn params_from_regulation(bytes: &[u8]) -> anyhow::Result<HashMap<Param, Vec<u8>>> {
        ...
        for file in &res.files {
            // note: Maybe storing errors here would be better rather than failing fast
            let file_name = &file
                .name
                .split("\\")
                .last()
                ...
                .split(".")
                .nth(0)
                .context("Filename must've an extension")?;
    ...
    fn decrypt(cipher_text: &[u8]) -> anyhow::Result<Vec<u8>> {
